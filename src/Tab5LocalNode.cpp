...
class Tab5LocalNodeImpl
{
  ...
  private:
    ...
    bool encodePacketForAir(const meshtastic_MeshPacket &packet, uint8_t *out, size_t &outLen,
                            bool hasChannelHashOverride = false, uint8_t channelHashOverride = 0)
    {
        ...
        header.channel = hasChannelHashOverride ? channelHashOverride : currentAirChannelHash();
    ...
    bool decodePacketFromAir(const uint8_t *payload, size_t payloadLen, float snr, int32_t rssi, meshtastic_MeshPacket &packet)
    {
        ...
        lastRxAirChannelHash = header.channel;
    ...
    bool shouldSuppressSelfEcho(const meshtastic_MeshPacket &packet) const
    {
        if (packet.from != localNodeNum() || packet.which_payload_variant != meshtastic_MeshPacket_decoded_tag) {
            return false;
        }
    ...
    bool shouldPlayNotificationSound(const meshtastic_MeshPacket &packet) const
    {
        if (packet.from == 0 || packet.from == localNodeNum() || packet.which_payload_variant != meshtastic_MeshPacket_decoded_tag) {
            return false;
        }
        ...
        return deviceUiConfig.alert_enabled;
    }

    const char *effectiveNotificationRingtone() const
    {
        if (currentRingtone[0] == '\0' || std::strncmp(currentRingtone, "Silent:", 7) == 0) {
            return ringtone[1].rtttl;
        }
    ...
    bool shouldSendRoutingAck(const meshtastic_MeshPacket &packet) const
    {
        ...
        return packet.to == localNodeNum() || packet.to == 0;
    }

    bool shouldRebroadcastPacket(const meshtastic_MeshPacket &packet) const
    {
        if (packet.from == 0 || packet.from == localNodeNum() || packet.to != UINT32_MAX || packet.hop_limit == 0) {
            return false;
        }
    ...
    void rebroadcastPacket(const meshtastic_MeshPacket &packet)
    {
        meshtastic_MeshPacket rebroadcast = packet;
        rebroadcast.hop_limit = packet.hop_limit > 0 ? packet.hop_limit - 1 : 0;
        rebroadcast.next_hop = 0;
        rebroadcast.relay_node = static_cast<uint8_t>(localNodeNum() & 0xFF);
        logPacketSummary("TX-RELAY", rebroadcast);
        transmitMeshPacket(rebroadcast, true, lastRxAirChannelHash);
    }

    bool shouldSendTraceRouteReply(const meshtastic_MeshPacket &packet) const
    {
        if (packet.from == 0 || packet.from == localNodeNum() || packet.to != localNodeNum() ||
            packet.which_payload_variant != meshtastic_MeshPacket_decoded_tag) {
            return false;
        }

        return packet.decoded.portnum == meshtastic_PortNum_TRACEROUTE_APP && packet.decoded.want_response;
    }
    ...
    void sendRoutingAck(const meshtastic_MeshPacket &request, meshtastic_Routing_Error error = meshtastic_Routing_Error_NONE)
    {
        ...
        transmitMeshPacket(scratchTxPacket, true, lastRxAirChannelHash);
    }

    void sendTraceRouteReply(const meshtastic_MeshPacket &request)
    {
        meshtastic_RouteDiscovery route = meshtastic_RouteDiscovery_init_default;
        if (request.decoded.payload.size > 0) {
            pb_decode_from_bytes(request.decoded.payload.bytes, request.decoded.payload.size, &meshtastic_RouteDiscovery_msg, &route);
        }

        if (route.route_count < sizeof(route.route) / sizeof(route.route[0])) {
            route.route[route.route_count++] = localNodeNum();
        }
        if (route.snr_towards_count < sizeof(route.snr_towards) / sizeof(route.snr_towards[0])) {
            route.snr_towards[route.snr_towards_count++] = static_cast<int8_t>(lroundf(request.rx_snr * 4.0f));
        }
        ...
        scratchTxPacket.which_payload_variant = meshtastic_MeshPacket_decoded_tag;
        scratchTxPacket.decoded = meshtastic_Data_init_default;
        scratchTxPacket.decoded.portnum = meshtastic_PortNum_TRACEROUTE_APP;
        scratchTxPacket.decoded.request_id = request.id;
        scratchTxPacket.decoded.dest = request.from;
        scratchTxPacket.decoded.source = localNodeNum();
        ...
        const size_t encoded =
            pb_encode_to_bytes(scratchTxPacket.decoded.payload.bytes, sizeof(scratchTxPacket.decoded.payload.bytes),
                               &meshtastic_RouteDiscovery_msg, &route);
        if (encoded == 0) {
            ILOG_WARN("TraceRoute reply encode failed for request 0x%08lx", static_cast<unsigned long>(request.id));
            return;
        }

        scratchTxPacket.decoded.payload.size = encoded;
        logPacketSummary("TX-TRACE-REPLY", scratchTxPacket);
        transmitMeshPacket(scratchTxPacket, true, lastRxAirChannelHash);
    }
    ...
    void transmitMeshPacket(const meshtastic_MeshPacket &packet, bool hasChannelHashOverride = false, uint8_t channelHashOverride = 0)
    {
        uint8_t payload[255] = {0};
        size_t encoded = 0;
        if (!encodePacketForAir(packet, payload, encoded, hasChannelHashOverride, channelHashOverride)) {
            ILOG_WARN("Meshtastic air encode failed");
            return;
        }

        if (hasChannelHashOverride) {
            ILOG_INFO("TX using response air channel hash override: 0x%02x", channelHashOverride);
        }
    ...
    void receiveFromRadio()
    {
        ...
        if (state == RADIOLIB_ERR_NONE) {
            if (decodePacketFromAir(payload, packetLength, snr, rssi, scratchRxPacket)) {
                logPacketSummary("RX", scratchRxPacket, packetLength);
                isRoutingAckForPending(scratchRxPacket);
                if (shouldSendRoutingAck(scratchRxPacket)) {
                    sendRoutingAck(scratchRxPacket);
                }
                if (shouldSendTraceRouteReply(scratchRxPacket)) {
                    sendTraceRouteReply(scratchRxPacket);
                }
                const bool duplicate = isDuplicateAirPacket(scratchRxPacket);
                if (!duplicate && shouldRebroadcastPacket(scratchRxPacket)) {
                    rebroadcastPacket(scratchRxPacket);
                }
                if (duplicate) {
                ...
                } else if (shouldSuppressSelfEcho(scratchRxPacket)) {
                    ILOG_INFO("Suppress self-echo text packet: from=0x%08lx id=0x%08lx",
                              static_cast<unsigned long>(scratchRxPacket.from),
                              static_cast<unsigned long>(scratchRxPacket.id));
                } else {
                    const bool playSound = shouldPlayNotificationSound(scratchRxPacket);
                    if (scratchRxPacket.which_payload_variant == meshtastic_MeshPacket_decoded_tag &&
                        scratchRxPacket.decoded.portnum == meshtastic_PortNum_TEXT_MESSAGE_APP) {
                        ILOG_INFO("TAB5 notification rx text from=0x%08lx id=0x%08lx alert=%d banner=%d playSound=%d ringtone='%.32s'",
                                  static_cast<unsigned long>(scratchRxPacket.from),
                                  static_cast<unsigned long>(scratchRxPacket.id),
                                  deviceUiConfig.alert_enabled ? 1 : 0,
                                  deviceUiConfig.banner_enabled ? 1 : 0,
                                  playSound ? 1 : 0,
                                  currentRingtone);
                    }
                    if (playSound) {
                        const char *notificationRingtone = effectiveNotificationRingtone();
                        ILOG_INFO("TAB5 notification play rtttl='%.32s'", notificationRingtone);
                        tab5PlayNotificationRtttl(notificationRingtone);
                    }
    ...
    bool updateKnownNode(uint32_t nodeNum, float snr)
    {
        if (nodeNum == 0) {
            return false;
        }
        ...
        if (it == knownNodes.end()) {
            ...
            node.snr = snr;
            knownNodes.push_back(node);
            pendingKnownNodeSave = true;
            pendingKnownNodeSaveAtMs = millis() + 2000;
            return true;
        }
    ...
    void processPendingKnownNodeSave()
    {
        if (!pendingKnownNodeSave) {
            return;
        }

        if (static_cast<int32_t>(millis() - pendingKnownNodeSaveAtMs) < 0) {
            return;
        }

        pendingKnownNodeSave = false;
        saveState();
    }
    ...
    bool pendingKnownNodeSave = false;
    uint32_t pendingKnownNodeSaveAtMs = 0;
    bool hasLearnedChannelHash = false;
    uint8_t learnedChannelHash = 0;
    uint8_t lastRxAirChannelHash = 0;
    char currentRingtone[231] = {0};
